...
@Controller
@RequestMapping("/safety-changed-redo")
public class ScRedoController extends HeaderCommonController {
    @Autowired
    ...
    public JSONObject infoTpl() {
        ...
        res = melApiFeign.dataModel(ApplicationConstain.SERVICE_ID, "detail-sinspect", "BROWSER");
        this.dealException(res);
        return res.getJSONObject("result");
    }
    ...
    public String list(Map<String,Object> data, 
        ...
        data.put("tpl", groupTpl().getJSONArray("groups").getJSONObject(2));
        PageRequest pr = getPageRequestData();
        Integer page = pr.getCurrentPage();
        Integer limit = pr.getLimit();
        JSONObject res = securityCheckApi.redoPage(ecmId, null, null, hiddenDangerGrade, null, null, keyword, page, limit);
